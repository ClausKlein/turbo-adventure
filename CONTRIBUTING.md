...
configure beman.my_project_name to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_MY_PROJECT_NAME`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_MY_PROJECT_NAME" -C 2
```
...
### `BEMAN_MY_PROJECT_NAME_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.my_project_name` can use
`find_package(beman.my_project_name)` to locate the library.
